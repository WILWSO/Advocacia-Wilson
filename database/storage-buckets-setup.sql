...
DROP POLICY IF EXISTS "Usuarios pueden leer documentos de clientes" ON storage.objects;
DROP POLICY IF EXISTS "Usuarios pueden eliminar documentos de clientes" ON storage.objects;
DROP POLICY IF EXISTS "Usuarios autenticados pueden subir documentos de processos" ON storage.objects;
DROP POLICY IF EXISTS "Usuarios pueden leer documentos de processos" ON storage.objects;
DROP POLICY IF EXISTS "Admin y advogados pueden eliminar documentos de processos" ON storage.objects;

-- 5. Políticas RLS para bucket foto_perfil
...
-- 6. Políticas RLS para bucket documentos_cliente
...
-- 7. Políticas RLS para bucket documentos_processo
