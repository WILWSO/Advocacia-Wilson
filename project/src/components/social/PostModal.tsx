...
import { X, Calendar, Tag, Play, FileText, Video, Image as ImageIcon, ExternalLink, Heart, MessageCircle, Share2, Send } from 'lucide-react';
...
const PostModal: React.FC<PostModalProps> = ({ post, isOpen, onClose }) => {
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        setNewComment('');
        setCommentAuthor('');
        alert('Comentário enviado! Será exibido após aprovação.');
      }
    } catch (err) {
  ...
  // Obtener ícono según tipo de post
  ...
  // Obtener color según tipo de post
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 pt-6 border-t border-neutral-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="text-sm text-neutral-500 flex items-center gap-1">
                      <Calendar size={14} />
                      {formatDate(post.data_publicacao || post.data_criacao)}
                    </div>
                  </div>
                </div>
              </div>
